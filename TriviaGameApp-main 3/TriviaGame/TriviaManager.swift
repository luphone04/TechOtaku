...
class TriviaManager: ObservableObject {
    ...
    init() {
        Task.init {
            await fetchTriviaFromLocalJSON()
        }
    }
...
//        }
//    }
    func fetchTriviaFromLocalJSON() async{
        if let url = Bundle.main.url(forResource: "TriviaQuestions", withExtension: "geojson") {
            do {
                let data = try Data(contentsOf: url)
                let jsonString = String(data: data, encoding: .utf8)
                print("GeoJSON Data:\n\(jsonString ?? "")") // Print the GeoJSON data for debugging
                
                let decoder = JSONDecoder()
                decoder.keyDecodingStrategy = .convertFromSnakeCase
                let decodedData = try decoder.decode(Trivia.self, from: data)

                // Rest of your code to reset variables and set new values
                DispatchQueue.main.async {
                    self.index = 0
                    self.score = 0
                    self.progress = 0.00
                ...
                print("Error loading local GeoJSON data: \(error)")
            }
        } else {
...
//                        // Handle the GeoJSON geometry as needed
//                        print(geometry)
//                    }
//                }
//            } catch {
